...
> 当前仓库已进入 **MVP 主流程后半段**。
> 首页、登录、采访、预览、我的书稿主链路已可演示，服务端也已具备用户鉴权与 SQLite 持久化；云托管测试环境已上线，真实微信登录、Doubao LLM 与火山语音 ASR 已完成联调，支付尚未接入。
...
4. 在进入商业化前完成真机验收、回归测试和云端持久化方案确认
...
- `docs/cloudbase-run.md`：微信云托管部署与联调说明
...
3. 做一轮真机主链路验收，确认云托管调用、微信登录、语音转写和预览生成稳定
4. 最后再接支付、会员、导出和上线准备
...
- `talkbook-api`：已部署到微信云托管，当前环境为 `talkbook-prod-d1gcxuvsbd9cb6803`
